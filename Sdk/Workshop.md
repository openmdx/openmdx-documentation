...
**IMPORTANT: This manual is out of date. It is currently being adapted to the gradle based build process of openmdx-v2.17.9.**
...
* __Java interfaces__: The model is mapped to Java interfaces according to the mapping specified
  ...
  After running _ant deliverables_ you will find these files in the _./build/jre-1.8/src/java_. The 
...
* __JPA classes__: Although the model specifies the _API_ of an application, it is a quite common use case that
  ...
  _orm.xml_ is generated in _build/jre-1.8/src/resource/META-INF/_. As explained later in this guide, 
...
All files generated by the mapper are stored in a _ZIP_ which is located in _build/jre-1.8/model/models.zip_. 
...
* Running _ant deliverables_ generates a _JPA_-compliant _orm.xml_ which is stored in 
  _build/jre-1.8/src/resource/META-INF/orm.xml_.
* Running _ant -Ddatabase.name="hsqldb-2" create-schema"_ creates a database-independent schema 
  file which is stored _build/jre-1.8/src/sql/openjpa-schema.xml_. 
...
* Running _ant -Ddatabase.name="hsqldb-2" create-sql_ generates the schema scripts for _HSQLDB_. 
  The output goes to _build/jre-1.8/src/sql/_.
